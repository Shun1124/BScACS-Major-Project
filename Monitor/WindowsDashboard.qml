...
Window { // Creates the main application window
    ...
    title: qsTr("System Monitor") // Sets the window title
    ...
    function addCriticalChange(message) { // Function to add a critical change notification and start the timer
        ...
        let formattedMessage = "[CRITICAL] Rollback performed for key: " + message; // Formats the critical change message
    ...
    ScrollView { // Main scrollable container for the application content
        ...
        Column { // Column layout for stacking UI components vertically
            ...
            Text { // Text item for displaying the title
                text: "Windows Registry Monitor" // Sets the text to be displayed
            ...
            Row { // Row layout for holding columns for logs and critical changes
                                    ...
                                    delegate: Rectangle { // Delegate defines how each item in the ListView will be displayed
                                        ...
                                        Row { // Row layout for displaying critical change text and button
                                            ...
                                            Text { // Displays the text for the critical change
                                                text: modelData.changeText.split("\\").pop() || "" // Shows the last part of the registry path
                                            ...
                                            Button { // Button for acknowledging the critical change
                                                ...
                                                onClicked: { // Action when button is clicked
                                                    ...
                                                    if (keyName) {
                                                        Monitoring.allowChange(keyName.trim()); // Calls Monitoring to allow change for this key
                                                    }
                ...
                Column { // Second column containing monitored registry keys and settings
                    ...
                    Rectangle { // Container for monitored registry keys
                        ...
                        height: 150 // Sets height for the registry keys container
                        ...
                        Column { // Column layout for registry keys list
                            ...
                            Text { // Label for the registry keys section
                                text: "Monitored Registry Keys:" // Label text
                            ...
                            ScrollView { // Scrollable container for registry keys list
                                id: registryScrollView // Unique identifier for the ScrollView
                                width: parent.width // Matches width to parent
                                ...
                                ListView { // ListView to display registry keys
                                    id: keyListView // Unique identifier for the ListView
                                    width: parent.width // Matches width to parent
                                    model: Monitoring ? Monitoring.registryKeys : [] // Sets model to registry keys in Monitoring object

                                    delegate: Rectangle { // Delegate defining the display of each registry key item
                                        width: keyListView.width // Matches the ListView width
                                        height: 30 // Sets height for each registry key item
                                        ...
                                        Row { // Row layout for each registry key item
                                            ...
                                            CheckBox { // CheckBox to mark registry keys as critical or non-critical
                                                ...
                                                text: model.displayText // Sets checkbox label to registry key name
                                                ...
                                                onCheckedChanged: { // Action when checkbox state changes
                                                    Monitoring.setKeyCriticalStatus(model.name, checked); // Calls method to update critical status
