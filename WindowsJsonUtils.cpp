#include "WindowsJsonUtils.h" // Includes the header file for JsonUtils functions
...
QList<RegistryKey*> readKeysFromJson(const QString &filePath) { // Function to read registry keys from a JSON file and return them as a list
    ...
    QList<RegistryKey*> registryKeys; // Initializes an empty list to store RegistryKey pointers
    ...
    for (const QJsonValue &value : keysArray) { // Iterates through each item in the JSON array
        QJsonObject obj = value.toObject(); // Converts each item to a JSON object
        QString hive = obj["hive"].toString(); // Extracts the "hive" string from the JSON object
        QString keyPath = obj["keyPath"].toString(); // Extracts the "keyPath" string from the JSON object
    ...
    qDebug() << "Registry Keys model size:" << registryKeys.size(); // Logs the size of the populated registry keys list
    return registryKeys; // Returns the list of registry keys
}
